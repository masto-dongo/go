...
import { DOMParser } from 'mastodon-go/util/polyfills';
import {
  Emoji,
  Emojifier,
} from 'mastodon-go/util/emoji';

export function ParseEmoji (html) {
  ...
  while (walker.nextNode()) {
    ...
    //  Otherwise we just push the (emojified) text.
    default:
      text.push(text, ...Emojifier.parse(node.textContent).map(
        stringOrEmoji => {
          if (stringOrEmoji instanceof Emoji) {
            return stringOrEmoji.toReact();
          }
          return stringOrEmoji;
        }
      ));
    }
  }
